...
2. **Create an API key in WooCommerce**  
   ...
   - Click **Add key**.  
   - Description: e.g. “Caliber Labs site”.  
   - User: choose an admin or a user that can create orders.  
   - Permissions: **Read/Write** (needed to create orders).  
   - Copy the **Consumer key** and **Consumer secret**.

3. **Add environment variables**  
   In the Caliber Labs project, create or edit `.env.local` (and add the same in your production host, e.g. Vercel):
   ...
   Optional (for full checkout page so order bumps work):

   ```env
   WOOCOMMERCE_CHECKOUT_MODE=checkout
   WOOCOMMERCE_CART_SECRET=your-long-random-secret
   ```
   ...
   - **WOOCOMMERCE_CHECKOUT_MODE** — Omit or `order-pay` = redirect to pay-for-order page; `checkout` = redirect to full checkout page (requires cart-redirect endpoint and **WOOCOMMERCE_CART_SECRET**).  
   - **WOOCOMMERCE_CART_SECRET** — Required when `WOOCOMMERCE_CHECKOUT_MODE=checkout`; same value as `CALIBER_CART_SECRET` in WordPress.  
...
5. **What happens at checkout**  
   ...
   - **Default (order-pay):** The API creates a **pending** order and returns the **order-pay** URL. The customer is sent to `/checkout/order-pay/123/...` to pay.  
   - **Full checkout (order bumps):** Set `WOOCOMMERCE_CHECKOUT_MODE=checkout` and install the cart-redirect endpoint (see below). The customer is sent to your store’s **full checkout page** (`/checkout/`) with the cart pre-filled, so order-bump and other checkout plugins work.  
   - If the env vars are not set, the app shows the demo “Thank you” message instead.

6. **Using the full checkout page (order bumps)**  
   If you want order bumps or other checkout-page plugins to run:
   - Set in `.env.local` (and production):  
     `WOOCOMMERCE_CHECKOUT_MODE=checkout`  
     `WOOCOMMERCE_CART_SECRET=your-long-random-secret`  
   - On WordPress (e.g. in `wp-config.php`):  
     `define( 'CALIBER_CART_SECRET', 'your-long-random-secret' );`  
   - Install the cart-redirect endpoint: copy `docs/wordpress-caliber-cart-redirect.php` into `wp-content/mu-plugins/caliber-cart-redirect.php`.  
   - Customers will hit your store’s `/checkout/` page with their cart already populated, so any order-bump or checkout customizer will apply.
