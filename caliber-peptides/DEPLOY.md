...
|-------|---------------------|-----|
| **Wrong directory** | You run `npm run dev` from the repo root (`Caliber Peptides`) instead of the app folder. | Run all npm commands from **`caliber-peptides`** (the folder that has `package.json` and `app/`). |
...
| **Dependencies not installed** | `node_modules` missing or broken. | From **`caliber-peptides`**: `npm install` then `npm run dev`. |
...
cd "/Users/jacobarnold/Caliber Peptides/caliber-peptides"
